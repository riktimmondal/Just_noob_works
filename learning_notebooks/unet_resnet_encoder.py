...
class EncoderDecoder(Model):
	...
	def predict(self, x):
        ...
        Args:
        	x: 4D torch tensor with shape (batch_size, classes, height, width)
        """
        if self.training():
        	self.eval()
...
class UnetDecoder(Model):
	...
	def __init__(self, encoder_channels, decoder_channels=(256, 128, 64, 32, 16),final_channels = 1,use_batchnorm= True,center = False,):
...
class Unet(EncoderDecoder):
    ...
    """
    def __init__(self, encoder_name='resnet34', encoder_weights='imagenet', decoder_use_batchnorm=True, decoder_channels=(256,128,64,32,16),classes=1,activation='sigmoid',center=False):
    	encoder = get_encoder(encoder_name, encoder_weights=encoder_weights)
    	decoder = UnetDecoder(encoder_channels=encoder.out_shapes, decoder_channels=decoder_channels, final_channels=classes, use_batchnorm=decoder_use_batchnorm, center=center)
